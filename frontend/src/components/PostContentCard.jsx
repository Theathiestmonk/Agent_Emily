  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className={`text-[11px] px-2 py-0.5 rounded-full border ${getStatusColor(badgeStatusKey)}`}>
          {statusLabel}
        </span>
      </div>
      ...
      <div className="p-4 space-y-3">
        {post.generated_caption && (
          <div className={`text-sm leading-relaxed line-clamp-4 whitespace-pre-wrap ${isDarkMode ? 'text-gray-200' : 'text-gray-700'}`}>
            <ReactMarkdown
              ...
              components={{
                ...
                code: ({ node, ...props }) => (
                  <code className={`px-1 rounded ${isDarkMode ? 'bg-white/10 text-white' : 'bg-gray-100 text-gray-900'}`} {...props} />
                )
              }}
            >
              {post.generated_caption}
        ...
        {normalizedHashtags.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {normalizedHashtags.slice(0, 8).map((tag, index) => (
              <span
                key={index}
                className={`text-xs font-semibold uppercase tracking-wider px-2 py-1 rounded-full border ${
                  isDarkMode
            ...
            {normalizedHashtags.length > 8 && (
              <span className="text-xs text-gray-400">+{normalizedHashtags.length - 8} more</span>
            )}
          </div>
        )}
        ...
        <div className={`flex items-center gap-2 mt-3 pt-3 border-t ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
          <button
            onClick={() => onApprove && onApprove(post.id)}
            className={`flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all border bg-transparent ${
              isDarkMode
