...
const ATSNChatbot = ({ externalConversations = null }) => {
  ...
  const loadConversations = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/chatbot/conversations?agent=atsn`, {
        method: 'GET',
        headers: {
    ...
    try {
      ...
      if (data.success && data.conversations && data.conversations.length > 0) {
        ...
        if (conversationMessages.length > 0) {
          setMessages(conversationMessages)
          console.log(`Loaded ${conversationMessages.length} today's ATSN conversations`)
        }
      }
    } catch (error) {
                            ...
                            <div className="overflow-x-auto overflow-y-hidden max-w-full" style={{
                              ...
                              <div className="chat-scrollbar flex gap-4 pb-2 px-1" style={{
                                ...
                                {message.content_items.map((contentItem, index) => (
                                  <div key={`${message.id}-${index}`} className="relative flex-shrink-0 w-80 transform hover:scale-105 transition-transform duration-200">
